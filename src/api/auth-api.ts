...
export class AuthApi extends BaseApi {
  ...
  public fetchUserData = async (username: string, password: string): Promise<IUser> => {
    ...
    switch (response.data?.username) {
      case "demo@demo.lv":
        return { username: response.data?.username, role: "admin", authenticated: true };
      case "demo1@demo.lv":
        return { username: response.data?.username, role: "user", authenticated: true };
      default:
